...
### 터미널 강제 종료 감지 (CWD 기반 프로세스 모니터링)
터미널 창을 X 버튼으로 강제 종료하면 SessionEnd 훅이 발동하지 않는 문제가 있음.
이를 해결하기 위한 가벼운 대안:
- **아이디어**: 매 3~5초마다 OS(Windows WMI 등)를 통해 실행 중인 `node.exe` 프로세스의 커맨드라인(CWD)을 검사
- JSONL 로그에 기록된 `cwd`를 잡고 있는 node 프로세스가 없다면 터미널이 닫힌 것으로 간주하고 즉시 에이전트 제거
- 장점: 앱 래핑(Wrapping)이나 무거운 파일 핸들(File Handle) 검사 없이, OS 레벨에서 확실하고 빠르게 비정상 종료를 감지 가능
